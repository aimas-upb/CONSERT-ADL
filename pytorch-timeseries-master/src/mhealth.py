...
def format_data_x(X):
    ...
    for i in range(len(X)):
        row = np.asarray(X[i, :])
        row = row.reshape(23, 10).T
        if X_new is None:
            X_new = np.zeros((len(X), 10, 23))
        X_new[i] = row
    print(X_new.shape)
    return X_new

def load_mhealth():
    X_all = np.load('../data/MHEALTH_X_all.npy')
    y_all = np.load('../data/MHEALTH_Y_all.npy')
    X_all = format_data_x(X_all)
...
def load(batch_size=64):
    ...
    x_train, x_val, x_test = x_train.reshape(
         (-1, 23, 10)), x_val.reshape(
         (-1, 23, 10)), x_test.reshape(
         (-1, 23, 10))
...
class MHEALTHTrainer(BaseTrainer):
    ...
    def save_model(self, savepath: Optional[Path] = None) -> Path:
        ...
        if savepath is None:
            model_name = f'{self.model.__class__.__name__}_model_mhealth_20_epochs.pkl'
...
def load_mhealth_trainer(model_path: Path) -> MHEALTHTrainer:
    ...
    loaded_trainer = MHEALTHTrainer(model)
