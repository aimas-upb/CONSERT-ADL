...
def format_data_x(X):
    ...
    for i in range(len(X)):
        row = np.asarray(X[i, :])
        row = row.reshape(120, 1).T
        if X_new is None:
            X_new = np.zeros((len(X), 1, 120))
        X_new[i] = row
    print(X_new.shape)
    return X_new

def load_wHAR():
    X_train = genfromtxt('../data/w-HAR Dataset/train_data.csv', delimiter=',')
    y_train = genfromtxt('../data/w-HAR Dataset/train_labels.csv', delimiter=',') - 1
    X_val = genfromtxt('../data/w-HAR Dataset/val_data.csv', delimiter=',')
    y_val = genfromtxt('../data/w-HAR Dataset/val_labels.csv', delimiter=',') - 1
    X_test = genfromtxt('../data/w-HAR Dataset/test_data.csv', delimiter=',')
    y_test = genfromtxt('../data/w-HAR Dataset/test_labels.csv', delimiter=',') - 1
...
def load(batch_size=64):
    x_train, y_train, x_val, y_val, x_test, y_test = load_wHAR()
    ...
    x_train, x_val, x_test = x_train.reshape(
         (-1, 120, 1)), x_val.reshape(
         (-1, 120, 1)), x_test.reshape(
         (-1, 120, 1))
...
class wHARTrainer(BaseTrainer):
    ...
    def save_model(self, savepath: Optional[Path] = None) -> Path:
        ...
        if savepath is None:
            model_name = f'{self.model.__class__.__name__}_model_wHAR_20_epochs.pkl'
...
def load_whar_trainer(model_path: Path) ->wHARTrainer:
    ...
    loaded_trainer = wHARTrainer(model)
