...
def format_data_x(X):
    ...
    for i in range(len(X)):
        row = np.asarray(X[i, :])
        row = row.reshape(40, 1).T
        if X_new is None:
            X_new = np.zeros((len(X), 1, 40))
        X_new[i] = row
    print(X_new.shape)
    return X_new

def load_wisdm():
    X_train = np.load('../data/WISDM_X_train.npy')
    y_train = np.load('../data/WISDM_Y_train.npy')
    X_val = np.load('../data/WISDM_X_val.npy')
    y_val = np.load('../data/WISDM_Y_val.npy')
    X_test = np.load('../data/WISDM_X_test.npy')
    y_test = np.load('../data/WISDM_Y_test.npy')
    print(X_train.shape)
    print(y_train.shape)
    print(X_val.shape)
    print(y_val.shape)
...
def load(batch_size=64):
    ...
    x_train, x_val, x_test = x_train.reshape(
        (-1, 3, 180)), x_val.reshape(
        (-1, 3, 180)), x_test.reshape(
        (-1, 3, 180))
...
class WISDMTrainer(BaseTrainer):
    ...
    def save_model(self, savepath: Optional[Path] = None) -> Path:
        ...
        if savepath is None:
            model_name = f'{self.model.__class__.__name__}_model_wisdm_20_epochs.pkl'
...
def load_wisdm_trainer(model_path: Path) -> WISDMTrainer:
    ...
    loaded_trainer = WISDMTrainer(model)
